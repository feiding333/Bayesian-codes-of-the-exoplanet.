log_post <- function(for_post){
  ...
  y <- for_post$y
  sd <- for_post$sd
  sigmaJ <- for_post$sigmaJ # Note changed to original scale to avoid alternating normalizing constant
  C <- for_post$C
  ...
  if (prod(is.finite(COV.obs.obs)) == 1){ # Deal with numerical problems
    ...
    log_normalizing <- -0.5*determinant(COV.obs.obs, logarithm=TRUE)$modulus
